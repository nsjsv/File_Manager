...
- **更完整的右键菜单和打开方式管理**：复制路径、更多打开方式、自定义动作、压缩/解压入口等还需要系统化整理。
- **批量工具**：批量重命名、批量改权限、批量属性检查等高频高级操作仍然缺位。
- **搜索索引管理**：索引排除规则、增量维护、重建入口、索引状态可视化还需要继续完善。
- **网络和远程位置**：目前重点在本地文件系统，SFTP、SMB、WebDAV 等远程位置还不是主要目标。
- **更成熟的文件操作审计**：后台队列和撤销/重做已经有基础，但跨会话历史、失败重试、详细日志和更完整的恢复策略仍需加强。
- **发行版和桌面环境覆盖**：主要开发和测试环境仍偏向 Linux / Wayland，其它发行版、X11、不同桌面环境下的兼容性还需要更多反馈。
